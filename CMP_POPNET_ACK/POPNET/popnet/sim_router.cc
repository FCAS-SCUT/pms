...
extern RouterIdMapping IdMapping[100]; //ljh
		            ...
		            if(MessageInfo[j].src1 == s1 && MessageInfo[j].src2 == s2 && MessageInfo[j].messageNo_m == msgNo)
		 	        {
                        ...
                        outputID = d1*8+d2;
                        sprintf(fileContents,"%s%d%s",filePath,outputID,".txt");
	        		    fp = fopen(fileContents,"a+");
                        while(fp == NULL){
        		    	    printf("file open error!\n");
        	    		    exit(0);
                        }
    		    	    if (flock(fp->_fileno, LOCK_EX) != 0) 
                            printf("The file is locked by others\n");
                        popnetCostTime = LATENCY_POPNET + cmpCostTime;
				        fprintf(fp, "%llu %llu %d %lld %d %lld %d %d %d ",sim_cycle,popnetCostTime, src_cmp, src_addr, dst_cmp, dst_addr, data_length, operation, msgNo);		        				
			 		    for(i=0; i<data_length; i++)
					    {
				            fprintf(fp, "%d ",mailbox[i]);
					    }
                        fprintf(fp,"\n");
				        fclose(fp);
