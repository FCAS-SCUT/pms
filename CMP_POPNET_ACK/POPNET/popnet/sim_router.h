...
//---------------------------------------------------------------------------//
		...
		void remove_flit(long a)
			...
			//changed at 2019-12-23
			  //flit_state_[a].erase(flit_state_[a].begin());
			  if(flit_state_[a].empty()){
				  //const char str_err[]="Error at output_template::remove_flit, sim_router.h\n";
				  cout<<"Error at output_template::remove_flit, sim_router.h\n"
				  	<<a<<endl;
				throw "Error at output_template::remove_flit, sim_router.h\n";
				//abort();
				//system("pause");
				//getchar();
			  }
			  else flit_state_[a].erase(flit_state_[a].begin());
			  localcounter_[a]++;}
