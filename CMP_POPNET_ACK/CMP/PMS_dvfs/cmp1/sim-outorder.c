...
#define ROUTERFILE    //------Tengfei Wang 20140226---//
#ifdef ROUTERFILE
    char *rFileFei;
#endif
    char *cycleFileFei;

/***********Tengfei Wang 20140325*********************
 *defination of input processor and L2 block dvfs file
 * **************************************************/
#define PROCESSORFILE
#ifdef PROCESSORFILE
    char *pFileFei;
#endif
...
/*********************ljh*********************/
/*int* read_mailbox(int size,md_addr_t addr)
{
     int mailbox[100];
     int *p;
     int data;
     p = mailbox;
     while(size--){ 
         data = MEM_READ(thecontexts[(current->regs).threadid]->mem, addr, int);
         *p++ = data;
         addr += sizeof(int);
         }
     p = mailbox;
...
sim_reg_options (struct opt_odb_t *odb)
{
	...
	//*****************************
	//add by ctb 2013-03-10
	opt_reg_string (odb, "-freq:proc", "the init frequency of processor", &proc_freq, /* default */ 0,
			/* print */ TRUE, /* format */ NULL);      

	opt_reg_string (odb, "-freq:router", "the init frequency of router", &router_freq, /* default */ 0,
			/* print */ TRUE, /* format */ NULL);
	opt_reg_string (odb, "-freq:l2", "the init frequency of l2", &l2_freq, /* default */ 0,
			/* print */ TRUE, /* format */ NULL);
...
#ifdef ROUTERFILE
    opt_reg_string(odb,"-sorted:router","frequency level of routers",&rFileFei,/*default*/"",/*print*/TRUE,/*format*/NULL );  //Tengfei Wang 20140226
#endif 
    opt_reg_string(odb,"-output:cycle","cycle output",&cycleFileFei,/*default*/"",/*print*/TRUE,/*format*/NULL );  //Tengfei Wang 20140226

#ifdef PROCESSORFILE
    opt_reg_string(odb,"-sorted:processor","frequency level of processors",&pFileFei,/*default*/"",/*print*/TRUE,/*format*/NULL );  //Tengfei Wang 20140325
#endif

#ifdef L2FILE
    opt_reg_string(odb,"-sorted:l2","frequency level of L2 blocks",&lFileFei,/*default*/"",/*print*/TRUE,/*format*/NULL );  //Tengfei Wang 20140325
#endif
		...
		int argc, char **argv)	/* command line arguments */
{
	...
	else			/* dl1 is defined */
	{
		...
		else
		{
			...
			cache_ml2 = cache_create (name, nsets, bsize, /* balloc */ TRUE,/* u


size */ 0, assoc, cache_char2policy (c),dl2_access_fn, /* hit lat */ cache_ml2_lat, -1);
...
sim_init (void)
{
	...
	{				
		...
		if (i == 0)
		{
			thecontexts[i]->power.aPowerfactor = (Mhz) * Vdd * Vdd;
			thecontexts[i]->power.aSensePowerfactor = (Mhz) * (Vdd / 2) * (Vdd / 2);
			thecontexts[i]->power.vdd_reduction = 1;
		}
		else
		{
			thecontexts[i]->power.aPowerfactor = (Mhz) * checkerVDD * Vdd * checkerVDD * Vdd;
			thecontexts[i]->power.aSensePowerfactor = (Mhz) * checkerVDD * (Vdd / 2) * checkerVDD * (Vdd / 2);
 ...
 * *************************************/
 #define MaxRouter 120
 #define SamplePointNum 50
...
long numrouters=100;
//char * freqindex[6]={"1.00","0.75","0.67","0.50","0.40","0.33"};
char * freqindex[7]={"1","0.75","0.67","0.5","0.4","0.25","0.1"}; //----Tengfei Wang-----//
char * freqindex_new[4]={"1","0.67","0.5","0.33"};
int num[7]={32,32,32,32,32,32,32};

float Real_IPC_Sample[SamplePointNum];
counter_t realsimcycle_backup=0,purenuminsn_backup=0,warm_cycle=0,warm_insn=0;
int IPC_Index=0;

void FindValue(char* fraction, FreqCounter * PF )
{
	if (!strcmp(fraction, "1"))
	{
		PF->Value = 1.00;
		PF->Nominator = 1;
		PF->Denominator = 1;
		PF->Vdd_reduction= 1.0;
		PF->latency_factor = 1.0;
	}
	else if (!strcmp(fraction, "0.75"))
	{
		PF->Value = 0.75;
		PF->Nominator = 3;
		PF->Denominator = 4;
		PF->Vdd_reduction= 0.9;
		PF->latency_factor = 1.3;
	}
	else if (!strcmp(fraction, "0.67"))
	{
		...
		PF->Nominator = 2;
		PF->Denominator = 3;
		PF->Vdd_reduction = 0.85;
		PF->latency_factor = 1.5;
	}
	else if (!strcmp(fraction, "0.5"))
	{
		PF->Value = 0.50;
		PF->Nominator = 1;
		PF->Denominator = 2;
		PF->Vdd_reduction = 0.8;
		PF->latency_factor = 2.0;
	}
	else if (!strcmp(fraction, "0.4"))
	{
		PF->Value = 0.40;
		PF->Nominator = 2;
		PF->Denominator = 5;
		PF->Vdd_reduction = 0.75;
		PF->latency_factor = 2.5;
	}
	else if (!strcmp(fraction, "0.33"))
	{
		...
		PF->Nominator = 1;
		PF->Denominator = 3;
		PF->Vdd_reduction = 0.7;
		PF->latency_factor = 3.0;
	}
	else if (!strcmp(fraction, "0.25"))
	{
		PF->Value = 0.25;
		PF->Nominator = 1;
		PF->Denominator = 4;
		PF->Vdd_reduction = 0.65;
		PF->latency_factor = 4.0;
	}
    else if(!strcmp(fraction, "0.2"))
    {
        PF->Value = 0.2;
		PF->Nominator = 1;
		PF->Denominator = 5;
		PF->Vdd_reduction = 0.6;
		PF->latency_factor = 5.0;
    }
    else if(!strcmp(fraction, "0.1"))       //------------Tengfei Wang20140226---------------//
    {
       PF->Value= 0.1;
       PF->Nominator = 1;
       PF->Denominator = 10;
       PF->Vdd_reduction = 0.5;
       PF->latency_factor = 10;
    }
	else
	{
		PF->Value=1.0;
...
void init_power( )
{
	...
	{				
		...
		FindValue(freqindex[0],&ProcFreq[i]);
		thecontexts[i]->power.aPowerfactor = (Mhz) * Vdd * Vdd * ProcFreq[i].Vdd_reduction * ProcFreq[i].Vdd_reduction * ProcFreq[i].Nominator / ProcFreq[i].Denominator ;
		thecontexts[i]->power.aSensePowerfactor = (Mhz) * (Vdd / 2) * (Vdd / 2) * ProcFreq[i].Vdd_reduction * ProcFreq[i].Vdd_reduction * ProcFreq[i].Nominator / ProcFreq[i].Denominator;
...
void setall_procfreq(int speed)
{
	int i;
	...
	{				
		...
		cache_il1[i]->hit_latency *= ProcFreq[i].latency_factor;
		cache_dl1[i]->hit_latency *= ProcFreq[i].latency_factor;
	}
}

void setall_routerfreq(int speed)
{
	int i;
...
inline int router_dvfs(int tID)
{
...
//**********************************************************************
//The function read_memory and write_memory is used to access the memory.
//**********************************************************************

void read_memory(int mailbox[], long data_length, md_addr_t src_addr)
{
      int *p;
      long size;
      size = data_length;
      ...
      //printf("The read data from mem are:\n");
      while(size--){
            mem_access_drct (thecontexts[0]->mem, Read, src_addr, p, sizeof(int), 0);
          //  printf("%d\t",*p);
            p++;   
            src_addr += sizeof(int);
      }
...
void write_memory(int data_length, md_addr_t dst_addr, int mailbox[])
{
    int *p,*q;
    p = mailbox;
    int a=0;
    ...
    //printf("The data write to mem are:\n");
    while(data_length--){
            mem_access_drct (thecontexts[0]->mem, Write, dst_addr, p, sizeof(int), 0);
            //mem_access_drct (thecontexts[0]->mem, Read, dst_addr, q, sizeof(int), 0);
            //printf("%d\t",*q);
            p++;
            dst_addr +=sizeof(int);
    }
	...
	while (tID < numcontexts)
	{
				...
				&& !done && !current->ruu_fetch_issue_delay && current->running && j < (ruu_decode_width * fetch_speed) && current->active_this_cycle /*&& ((ray_mult * (j + current->fetch_num) + current->RUU_num) < (RUU_size - 1))*/; i++, j++)
		{
			...
			if (current->ld_text_base <= current->fetch_regs_PC && current->fetch_regs_PC < (current->ld_text_base + current->ld_text_size) &&/*hq:I suggest remove "!"*/ !(current->fetch_regs_PC & (sizeof (md_inst_t) - 1)))
			{
               ...
               // printf("The current threadid is:%d      The current PC is:%lld\n",current->id,current->fetch_regs_PC);
...
#include "machine.def"
	...
	//srand((int)time(NULL));
	//random_num();
	//printf("the random frequency setting times: %d %d %d %d %d %d\n",num[0],num[1],num[2],num[3],num[4],num[5],num[6]);

//-----------TengfeiWang 20140225----------------------//
#ifdef ROUTERFILE
    FILE *rFreq_Figo;
	rFreq_Figo = fopen(rFileFei,"r");//fopen("./rFileFei.txt","r");
    if(!rFreq_Figo){
            printf("router frequency file open fail!\n");
            printf("default router frequency:1  /*111111111111111111111111111*/\n");
    }
    else{ 
       printf("router file open correctly/******Tengfei Wang******/!\n");
		while (!feof(rFreq_Figo) )
		{
            int i=0;
			char type ;
			char  freq[10];
			fscanf(rFreq_Figo,"%c %d %s\n", &type, &i,  freq);
			if (type == 'R' )
			{
				int Y=i/(mesh_size);
				int X=i%mesh_size;
				int k=Y*(mesh_size+2)+X;
				FindValue(freq,&RouterFreq[k]);
			}
          //  else if(type == 'P')                            /*******Tengfei Wang 0325************/
           // {  
            //    if(i<MAXTHREADS){ 
             //       FindValue(freq,&ProcFreq[i]);
            //    }
            //    else
            //    {   
            //        printf("processor id beyond the maxvalue!!\n");
           //         return 0;
           //     }
           // }
           // else if(type == 'M')
           // {   
            //    FindValue(freq,&L2Freq[i]);
           // }
        }
        fclose(rFreq_Figo);
    }
#endif
    /********************Tengfei Wang 20140325**************************/
    /*for dvfs of processors and L2 cache blocks
     * *****************************************************************/
#ifdef PROCESSORFILE
    FILE *pFreq_Figo;
	pFreq_Figo = fopen(pFileFei,"r");
    if(!pFreq_Figo){
            printf("processor frequency file open fail!\n");
            printf("default processor frequency:1  /*111111111111111111111111111*/\n");
    }
    else{ 
        printf("processor frequency file open correctly/******Tengfei Wang******/!\n");
		while (!feof(pFreq_Figo) )
        {
            int i=0;
            char type;
            char freq[10];
            fscanf(pFreq_Figo,"%c %d %s\n",&type,&i,freq);
            if(type == 'P'){
                if(i<MAXTHREADS){
                        FindValue(freq,&ProcFreq[i]);  
                }
                else{
                        printf("processor id is beyond the max value!!");
                        return 0;
                }
            }
...
#ifdef L2FILE
    FILE *lFreq_Figo;
	lFreq_Figo = fopen(lFileFei,"r");
    if(!lFreq_Figo){
            printf("L2 block frequency file open fail!\n");
            printf("default L2 blocks frequency:1  /*111111111111111111111111111*/\n");
    }
    else{ 
        printf("L2 block frequency file open correctly/******Tengfei Wang******/!\n");
		while (!feof(lFreq_Figo) )
        {
            int i=0;
            char type;
            char freq[10];
            fscanf(lFreq_Figo,"%c %d %s\n",&type,&i,freq);
            if(type == 'M'){
                if(i<MAXTHREADS){
                        FindValue(freq,&L2Freq[i]);  
                }
                else{
                        printf("L2 block id is beyond the max value!!");
                        return 0;
                }
            }
...
/*
	CTB_FILE=fopen(my_simout,"r");
	if(CTB_FILE==NULL)
	{
		CTB_FILE=fopen(my_simout,"w+");
		if(CTB_FILE!=NULL)
		{
			fprintf(CTB_FILE,"CYCLE    IPC    ");
			...
			if(random_proc)
			{
				for(i=0;i<numcontexts;i++)
				{
					fprintf(CTB_FILE,"P%d  ", i);
				}
			}
//#define Figo2      //---Tengfei Wang---//
#ifdef FIGO2
			if(random_router)
			{
				for(i=0;i<numcontexts;i++)
				{
					fprintf(CTB_FILE,"R%d  ", i);
				}
			}
#endif
			if(random_l2)
...
//#define Figo2
#ifdef FIGO2                            //----Tengfei Wang 20140225----for commenting--------//
	if(random_router)
	{
		for(i=0;i<numcontexts;i++)
		{
...
#define FREQ_NUM 4
		...
		for(j=0;j<mesh_size/col;j++)
		{
			...
			for(m=0;m<row;m++)
			{
				for(n=0;n<col;n++)
				{
					...
					FindValue(freqindex_new[tt],&ProcFreq[set_id]);
					FindValue(freqindex_new[tt],&RouterFreq[k]);
					FindValue(freqindex_new[tt],&L2Freq[set_id]);
	...
	if(test!=NULL)
	{
		...
		for(i=0;i<numcontexts;i++)
		{
			fscanf(test,"%s ", iid);
			int k=(i/mesh_size+1)*(mesh_size+2)+i%mesh_size;
			FindValue(iid,&RouterFreq[k]);

		}
		for(i=0;i<numcontexts;i++)
		{
			fscanf(test,"%s ", iid);
			FindValue(iid,&L2Freq[i]);
...
#define PRINT_STATISTICS 0
		...
		if (((!max_barrier && (pure_num_insn > executeMaxInsn)) || (max_barrier && TotalBarriers >= max_barrier)) && collect_stats)
		{
			...
			// ctb 2013-03-08
			//**********************************
			Real_IPC_Sample[IPC_Index]=(1.0*(pure_num_insn-purenuminsn_backup))/(realSimCycle-realsimcycle_backup);
			printf("RealTime_insn=%lld\tRealTime_simcycle=%lld\tRealTime_IPC=%f\n", pure_num_insn-purenuminsn_backup, realSimCycle-realsimcycle_backup, Real_IPC_Sample[IPC_Index]);
			...
			if(CTB_FILE!=NULL)
			{
				...
				fprintf(CTB_FILE,"\n%lld ",(sim_cycle-warm_cycle));
				fprintf(CTB_FILE,"%lld ",(IPC_Index));

				fprintf(CTB_FILE,"%8.5f ",(1.0*(pure_num_insn-warm_insn))/(sim_cycle-warm_cycle));
                fprintf(CTB_FILE,"%8.5f ",1.0*pure_num_insn/realSimCycle);
				fprintf(CTB_FILE,"%8.5f ",1.0*sim_num_insn/sim_cycle);

				if(random_proc)
				{
					for(i=0;i<numcontexts;i++)
					{
						fprintf(CTB_FILE,"%3.2f ", ProcFreq[i].Value);
					}
				}
				if(random_router)
				{
					...
					{
						int k=(i/mesh_size+1)*(mesh_size+2)+i%mesh_size;
						fprintf(CTB_FILE,"%3.2f ", RouterFreq[k].Value);
					}
				}
				if(random_l2)	
				{
					for(i=0;i<numcontexts;i++)
					{
						fprintf(CTB_FILE,"%3.2f ", L2Freq[i].Value);
					}
				}
		...
		if (unPrintLimit < STATUS_NUM && (pure_num_insn >= (unPrintLimit * (max_insts / STATUS_NUM))))
		{
			if (!mystricmp (network_type, "FSOI") || !mystricmp (network_type, "HYBRID"))
			{
                ...
                FILE *ffei=fopen(cycleFileFei,"a+");
				fprintf (ffei, "sim_cycle= %lld\tsim_num_insn=%lld\tProgress=%d\%\tdata_packets_generate=%d\n", sim_cycle, pure_num_insn, (int)((unPrintLimit * 100) / STATUS_NUM), global_data_packets_generation);           //-----Tengfei Wang 20140226-----------//
			    fclose(ffei);
            }
			else if (!mystricmp (network_type, "MESH"))
			{
                ...
                //---Tengfei Wang20140226-------//
                FILE *ffei=fopen(cycleFileFei,"a+");
                fprintf (ffei, "sim_cycle= %lld\t", sim_cycle);
                fprintf (ffei, "sim_num_insn=%lld\t", pure_num_insn);
                fprintf (ffei, "Progress=%d%\t", (unPrintLimit * 100) / STATUS_NUM);
                fprintf (ffei, "data_packets_generate=%d\n", DataPackets_1);
                fclose(ffei);
			}
			else if ((!mystricmp (network_type, "COMB")))
			{
				...
				//-----Tengfei Wang 20140226---//     
               FILE *ffei=fopen(cycleFileFei,"a+");
                fprintf (ffei, "sim_cycle= %lld\tsim_num_insn=%lld\tProgress=%d%\n", sim_cycle, pure_num_insn, (unPrintLimit * 100) / STATUS_NUM);
			    fclose(ffei);
            }
            fflush (stderr);
			fflush (stdout);
			...
			//calculate the ipc of each sample time
			if(IPC_Index==20)
			{
				warm_cycle=sim_cycle;
				warm_insn =pure_num_insn;
			}
			Real_IPC_Sample[IPC_Index]=(1.0*(pure_num_insn-purenuminsn_backup))/(realSimCycle-realsimcycle_backup);
			printf("RealTime_insn=%lld\tRealTime_simcycle=%lld\tRealTime_IPC=%f\n", pure_num_insn-purenuminsn_backup, realSimCycle-realsimcycle_backup, Real_IPC_Sample[IPC_Index]);
			fprintf(stderr, "RealTime_insn=%lld\tRealTime_simcycle=%lld\tRealTime_IPC=%f\n", pure_num_insn-purenuminsn_backup, realSimCycle-realsimcycle_backup, Real_IPC_Sample[IPC_Index]);
