...
#endif
				threadid IF_L2_COMM_LAT_CL (cluster), NULL);
...
#else
#ifndef PERFECT_CACHE
											lat = cache_access (cache_dl1[threadid], Write, (current->LSQ[LSQ_index].addr & ~3), NULL, 4, sim_cycle, NULL, NULL, NULL, threadid);
#else
											lat = cache_dl1_lat;
#endif
#endif
...
#ifdef REAL_LOCKn
					if(rs->isSyncInst && rs->op == STL && addr == 5371085676)
						printf("release the lock by threadid %d, at sim_cycle %lld\n", threadid, sim_cycle);
					if(rs->isSyncInst && rs->op == LDL_L && addr == 5371085676)
						printf("Acquireeeeeeeeeee the lock by threadid %d, at sim_cycle %lld\n", threadid, sim_cycle);
#endif
...
#include "machine.def"
				...
				if (MD_OP_FLAGS (op) & F_LOAD)
				{
					int lat;
#ifdef DIRECTORY
					int load_lat = cache_access (cache_dl1[threadid], Read, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, rs, 0, NULL);
#else
					int load_lat = cache_access (cache_dl1[threadid], Read, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, 0);
#endif
					if (dtlb)
#ifdef DIRECTORY
						lat = cache_access (dtlb[threadid], Read, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, NULL, 0, NULL);
#else
				...
				{
#ifdef DIRECTORY
					cache_access (cache_dl1[threadid], Write, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, rs, 0, NULL);
#else
					cache_access (cache_dl1[threadid], Write, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, 0);
#endif
					if (dtlb)
#ifdef DIRECTORY
						cache_access (dtlb[threadid], Read, (addr & ~3), NULL, 4, sim_num_insn, NULL, NULL, NULL, 0, NULL);
#else
