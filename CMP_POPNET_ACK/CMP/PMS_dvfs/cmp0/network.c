...
//	int flag = local_access_check(event->src1, event->src2, event->des1, event->des2);
	...
	if(!flag && collect_stats)
	{
		...
		if (!mystricmp (network_type, "FSOI"))
			...
			popnetMessageInsert(event->src1, event->src2, event->des1, event->des2, sim_cycle, meta_packet_size, popnetMsgNo, 0, event->opt, 0,0,0,0,0);
...
void scheduleThroughNetwork(struct DIRECTORY_EVENT *event, long start, unsigned int type, int vc)
{
	...
	if(event->src1 < 0 || event->src1 > 16 || event->src2 < 0 || event->src2 > 16 || event->des1 < -1 || event->des1 > 16 || event->des2 < -1 || event->des2 > 16)//zl
	...
	*/
	...
	{ /* mesh network schedule */
		...
		{ /*non-local accesses, go the network */
			...
			vc = 0;
	#endif
			popnetMessageInsert(event->src1, event->src2, event->des1, event->des2, start, type, popnetMsgNo, event->addr>>cache_dl2->set_shift, event->operation, vc,0,0,0,0,0);
		...
		if(!flag)
		{
			...
			else 
			{
				...
				if(Chip_identify == MESH_NET_INTER)
				{
					...
					vc = 0;
#endif
					popnetMessageInsert(new_src1, new_src2, new_des1, new_des2, start, type, popnetMsgNo, event->addr>>cache_dl2->set_shift, event->operation, vc,0,0,0,0,0);
#ifdef MULTI_VC
