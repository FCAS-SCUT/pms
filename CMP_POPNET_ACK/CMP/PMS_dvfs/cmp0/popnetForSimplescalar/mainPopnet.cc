...
//void popnetMessageInsert(long s1, long s2, long d1, long d2, long long int src_addr, long data_length, counter_t src_cmp, md_addr_t dst_addr, int operation, int dst_cmp)
...
void popnetMessageInsert(long s1, long s2, long d1, long d2, long long int sim_cycle, long size, counter_t msgNo, md_addr_t dst_cmp, int op, int vc,long long int src_addr, long data_length, counter_t src_cmp, md_addr_t dst_addr, int operation)
{
	...
	if(d1==0&&d2==0){
	    ...
	    MessageInfo[countNum].size_m = size;
        MessageInfo[countNum].vc_m = vc;
		...
		for(int i = 0; i < fileLine; i++){
			...
			if(ConvertStringToNum(filecontent[i],numResult)){	
				...
				if(operation_t == 1){ //operation equal to 1 is stand for other clusters want to write this cluster.
					...
					fout << event_time_t+1000 <<" " <<costCycle << " "  << dst_cmp <<" " << dst_addr_t <<" " << src_cmp << " " << src_addr_t <<" " << "1" <<" " << operation_t <<" " << messageNo_t*10+1 <<" "<<"111"<<endl;
                    fflush(stdout);
                    fout.close();
					...
					fout_b << event_time_t+1000 <<" " <<costCycle << " "  << dst_cmp <<" " << dst_addr_t <<" " << src_cmp << " " << src_addr_t <<" " << "1" <<" " << operation_t <<" " << messageNo_t*10+1 <<" "<<"111"<<endl;
                    fflush(stdout);
                    fout_b.close();
                }      
                else if(operation_t  == 2)
                {
                    sendFlag = 1;
                    printf("@@@@@@@@@@@@@@@@@@@@@@@@@@I received the ACK signal.@@@@@@@@@@@@@@@@@@@@@@@@@\n");
                }
