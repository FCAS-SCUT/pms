...
/***************************ljh**********************************/
int data_length,src_cmp,dst_cmp,operation;
int data;
md_addr_t dst_addr, src_addr;
extern counter_t packageDelay;
/***************************ljh**********************************/
...
extern counter_t popnetMsgNo;//huarzail
...
/*****************huarzail************************/
#define OSF_SYS_hello 			193
#define OSF_SYS_readfile 		194
#define OSF_SYS_write_mailbox		195
#define OSF_SYS_read_mailbox		196
#define OSF_SYS_mailboxsend		197
#define OSF_SYS_mailboxreceive		262
#define OSF_SYS_printresult		198
#endif
...
//	}
  ...
  bool success = false;
    ...
    case OSF_SYS_hello: 
      ...
      break ;
    case OSF_SYS_readfile:
	   {	
	int flag;		
	current->sleptAt = sim_cycle;
	flag = regs->regs_R[R_A0];
	QuiesceAddrStruct[current->id].flag = flag ;
	if (flag == 1){
		current->running = 0;
		current->freeze = 1;
	}	
...
/*************************ljh***********************/
    case OSF_SYS_mailboxsend: {

	    struct DIRECTORY_EVENT *event;
		extern md_addr_t src_addr;
		extern md_addr_t dst_addr;
        extern int src_cmp;
		extern int dst_cmp;
		extern int data_length;
		extern int operation;

        src_cmp = regs->regs_R[R_A0];
        src_addr = (md_addr_t)regs->regs_R[R_A1];
        dst_cmp = regs->regs_R[R_A2];
        dst_addr = (md_addr_t)regs->regs_R[R_A3];
        data_length = regs->regs_R[R_A4];
        operation = regs->regs_R[R_A5];
        popnetMsgNo++;
        //popnetMessageInsert(current->id/8+1, current->id%8, 0, 0, (md_addr_t)src_addr, data_length, (md_addr_t)src_cmp, (md_addr_t) dst_addr, operation, (md_addr_t)dst_cmp, 0);	
		popnetMessageInsert(current->id/8+1, current->id%8, 0, 0, sim_cycle, 5, popnetMsgNo, (md_addr_t) dst_cmp, 1, 0,(md_addr_t)src_addr, data_length, (md_addr_t)src_cmp, (md_addr_t) dst_addr, operation);	
	
    }break;


    case OSF_SYS_printresult:{
        counter_t  resultCycle;        
        resultCycle = sim_cycle - sleepTime - packageDelay;
	    //regs->regs_R[MD_REG_V0] = resultCycle;
	    //regs->regs_R[MD_REG_V0] = resultCycle;
	    //regs->regs_R[MD_REG_A3] = 0;

        printf("******The simulation result is:%llu******\n",sim_cycle);
        fflush(stdout);        
    }break;
...
/*		fprintf(stderr, "Thread %d is sleeping on %llx\n", current->id, regs->regs_R[MD_REG_A0]);
	...
	if (!current->masterid)
	{
	    ...
	    quiesceAddrStruct[current->id].sleepTimeStart = sim_cycle;
	    current->sleptAt = sim_cycle;
	    freezeCounter++;
	}
	regs->regs_R[MD_REG_V0] = 0;
