	...
	if(b.type() == TAIL_ || b.flit_size() == 1)
	{
		...
		if(y==0&&z==0)
		{
			for(j=0; j<countNum; j++)
				...
				FILE *fp_o;
				FILE *fp_b; // The buckup file is used for debug
                ...
                if (flock(fp_b->_fileno, LOCK_EX) != 0) 
					...
					if(operation == 0){      	
						fprintf(fp_o, "%llu %llu %d %lld %d %lld %d %d %d ",sim_cycle,simNetCycle, src_cmp, src_addr, dst_cmp, dst_addr, data_length, operation, msgNo);
						...
						fprintf(fp_b, "%llu %llu %d %lld %d %lld %d %d %d ",sim_cycle,simNetCycle, src_cmp, src_addr, dst_cmp, dst_addr, data_length, operation, msgNo);
						for(i=0; i<data_length; i++)
						{
