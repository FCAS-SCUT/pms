...
#include "memory.h"
	...
	static unsigned int proc_freq, router_freq, l2_freq; // init freq
...
sim_reg_options (struct opt_odb_t *odb)
{
	...
	//*****************************
	//add by ctb 2013-03-10
	opt_reg_uint (odb, "-freq:proc", "the init frequency of processor", &proc_freq, /* default */ 0,
			/* print */ TRUE, /* format */ NULL);

	opt_reg_uint (odb, "-freq:router", "the init frequency of router", &router_freq, /* default */ 0,
			/* print */ TRUE, /* format */ NULL);
	opt_reg_uint (odb, "-freq:l2", "the init frequency of l2", &l2_freq, /* default */ 0,
			/* print */ TRUE, /* format */ NULL);
...
char * freqindex_new[4]={"1.00","0.67","0.50","0.33"};
int num[6]={32,32,32,32,32,32};
...
#define FREQ_NUM 4
	...
	for (;;)
	{
/*--------2013-12-04-zhenglong------------*/
#ifdef FL

	FILE *fpread;
	int readfile,filecount;
	
 for (filecount=0;filecount<=63;filecount++)	
 {	 
	if (QuiesceAddrStruct[filecount].flag == 1)		
	{	
				//hardcode
		if((fpread = fopen("/home/w/huarzail/ALPHA_CMP/program/fileread/file.txt","r")) == NULL)
		{
		...
		{		
			thecontexts[filecount]->freeze = 0;
      			thecontexts[filecount]->running = 1;
			QuiesceAddrStruct[filecount].flag = 0;
			fclose(fpread);
...
#define PRINT_STATISTICS 0
		...
		busFree ();
	     ...
	     sim_cycle++;
/*******************************z1************************************
	while(sim_cycle % 1000 == 0)
        {
		FILE *sync_1_net, *sync_net_1;

		sync_1_net = fopen ("/home/w/huarzail/CMP_POPNET/sync_1_net.txt","w");
		sync_net_1 = fopen ("/home/w/huarzail/CMP_POPNET/sync_net_1.txt","r");

		if(sync_1_net == NULL && sync_net_1 == NULL) {
			printf("Sync is not ready!!");
		        while(1);
		}        
		fprintf(sync_1_net, "%lld", sim_cycle);
		fclose(sync_1_net);
		fscanf(sync_net_1, "%lld", &synctime); 
		fclose(sync_net_1);
		if(synctime==sim_cycle){
			break;	
		}
