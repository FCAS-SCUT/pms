	...
	if(fileLine){
		for(int i = 0; i < fileLine; i++){
			...
			if(ConvertStringToNum(filecontent[i],numResult)){	
				...
				if(operation_t == 1){ //operation = 1 is stand for other clusters want to write this cluster.
					...
					fout << event_time_t <<" " <<Time << " "  << dst_cmp <<" " << dst_addr_t <<" " << src_cmp << " " << src_addr_t <<" " << "1" <<" " << operation_t <<" " << messageNo_t <<" "<<"111"<<endl;
                    fflush(stdout);
                    fout.close();
					...
					fout_b << event_time_t <<" " <<Time << " "  << dst_cmp <<" " << dst_addr_t <<" " << src_cmp << " " << src_addr_t <<" " << "1" <<" " << operation_t <<" " << messageNo_t <<" "<< "111" <<endl;
                    fflush(stdout);
                    fout_b.close();
                }      
