...
int main()
{
	...
	MTA_Stats(1);
	{
		...
		int *localAddress3  =   (int *)cmp0_3;
		int *localAddress4  =   (int *)cmp0_3;
            ...
            if(my_id == 0){	
                *localAddress3 = *localSignal0;
                for(i=1; i<=LOCALNUM; i++)
                    *(localAddress3+i) = *(localAddress2+i);            
				printf("Data from cluster0 send to cluster1.\n");
				MTA_mailboxsend(0,localAddress4,1,otherAddress0,LOCALNUM+1,1);//send the data    
