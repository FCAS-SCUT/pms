...
void InitialDist(int coreNum,int array[],int x[],int *p, int *q){
	...
	if(LOCALNUM%coreNum == 0){
		...
		for(i=LOCALNUM;i<tmp+LOCALNUM;i++){
			array[i] = INF;	
        }
    *p = num;
    *q = tmp;    
...
int main()
{
	...
	MTA_Stats(1);//enter to kernal
	{
        ...
        *localSignal0 = 999;//send signal
        ...
        if(my_id == 0){
            printf("I receive the data beginning...\n"); 
            fflush(stdout);    
            ReceiveData(FinalArray);
		    MTA_printresult();
            WaitTime(8000); 
		    MTA_printresult();

			MergeSort(FinalArray,0,LOCALNUM-1,NODENUM);
        }
	...
	/*if(my_id == 1){
            ...
            fflush(stdout);
			WriteFile("./output.txt",FinalArray,NODENUM);
		}*/
	}
