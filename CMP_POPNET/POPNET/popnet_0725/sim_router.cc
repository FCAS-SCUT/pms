...
void sim_router_template::receive_packet()//zl
{
	time_type event_time = mess_queue::m_pointer().current_time();
	long cube_s = sim_foundation::wsf().cube_size();
	add_type sor_addr_t;
	...
	//packet_counter_++;
	msgNo = msgNoPass;
	//cout << "sim_router msgNo :" << msgNo ;			
	    inject_packet(/*packet_counter_*/msgNo, sor_addr_t, des_addr_t, local_input_time_, /*pack_size_t*/5);
	    //inject_packet(packet_counter_/*msgNo*/, sor_addr_t, des_addr_t, local_input_time_, /*pack_size_t*/5);
...
void sim_router_template::accept_flit(time_type a, const flit_template & b)// zl
{
    //	long long int  msgNo;
	...
	if(b.type() == TAIL_ ) {
		...
		long long int cmpCostTime;
		long long int popnetCostTime;
		int src_cmp;
		...
		if ((d1 == 0 && d2 == 0) && (s1 !=0 || s2 !=0)) {
		    for(j=0; j<countNum; j++)
				if(MessageInfo[j].src1 == s1 && MessageInfo[j].src2 == s2 && MessageInfo[j].messageNo_m == msgNo)
				{
				    ...
				    cmpCostTime   = MessageInfo[j].costCycle_m;
                
                for(i=0; i<data_length; i++)
                        mailbox[i] = MessageInfo[j].array[i];

                popnetCostTime = LATENCY_POPNET + cmpCostTime;

		        if ((fp = fopen("/home/w/huarzail/CMP_POPNET/pt_net_0.txt", "r")) == NULL){ 
				...
				ofstream fout("/home/w/huarzail/CMP_POPNET/pt_net_0.txt", ios::app);
				fout << sim_cycle << " " << popnetCostTime << " " << s1 << " "<< s2 << " " << src_addr << " " << d1 << " " << d2 << " "  << dst_addr << " " << data_length << " " << operation << " " << msgNo <<" ";			
					 for(i=0; i<data_length; i++)
					 {
				     fout << mailbox[i] << " ";
					 }
		 ...
		 if (d1 == 0 && d2 == 1 ) {
		     for(j=0; j<countNum; j++)
				if(MessageInfo[j].src1 == s1 && MessageInfo[j].src2 == s2 && MessageInfo[j].messageNo_m == msgNo)
				{
				    sim_cycle   = MessageInfo[j].sim_cycle_m;
				    src_cmp     = MessageInfo[j].src_cmp_m;
				    src_addr    = MessageInfo[j].src_addr_m;
				    dst_cmp	    = MessageInfo[j].dst_cmp_m;
				    dst_addr    = MessageInfo[j].dst_addr_m;
                ...
                for(i=0; i<data_length; i++)
                        mailbox[i] = MessageInfo[j].array[i];
                popnetCostTime = LATENCY_POPNET + cmpCostTime;
		        if ((fp = fopen("/home/w/huarzail/CMP_POPNET/pt_net_1.txt", "r")) == NULL){ 
        		    printf("file open error!\n");
        	    	exit(0);
    	    	}
    		    if (flock(fp->_fileno, LOCK_EX) != 0) 
				...
				ofstream fout("/home/w/huarzail/CMP_POPNET/pt_net_1.txt", ios::app);
				fout << sim_cycle << " " << popnetCostTime << " " << s1 << " "<< s2 << " " << src_addr << " " << d1 << " " << d2 << " "  << dst_addr << " " << data_length << " " << operation << " " << msgNo <<" ";			
					 for(i=0; i<data_length; i++)
					 {
				     fout << mailbox[i] << " ";
		        ...
		        if (d1 == 1 && d2 == 0 ) {
			     for(j=0; j<countNum; j++)
				if(MessageInfo[j].src1 == s1 && MessageInfo[j].src2 == s2)
				{
				    sim_cycle   = MessageInfo[j].sim_cycle_m;
				    src_cmp     = MessageInfo[j].src_cmp_m;
				    src_addr    = MessageInfo[j].src_addr_m;
                ...
                for(i=0; i<data_length; i++)
                        mailbox[i] = MessageInfo[j].array[i];
                popnetCostTime = LATENCY_POPNET+ cmpCostTime;
		        if ((fp = fopen("/home/w/huarzail/CMP_POPNET/pt_net_2.txt", "r")) == NULL){ 
        		    printf("file open error!\n");
        	    	exit(0);
    	    	}
    		    if (flock(fp->_fileno, LOCK_EX) != 0) 
				...
				ofstream fout("/home/w/huarzail/CMP_POPNET/pt_net_2.txt", ios::app);
				fout << sim_cycle << " " << popnetCostTime << " " << s1 << " "<< s2 << " " << src_addr << " " << d1 << " " << d2 << " "  << dst_addr << " " << data_length << " " << operation << " " << msgNo <<" ";			
					 for(i=0; i<data_length; i++)
					 {
				     fout << mailbox[i] << " ";
                ...
                for(i=0; i<data_length; i++)
                        mailbox[i] = MessageInfo[j].array[i];
                popnetCostTime = LATENCY_POPNET + cmpCostTime;
		if ((fp = fopen("/home/w/huarzail/CMP_POPNET/pt_net_3.txt", "r")) == NULL){ 
        		    //printf("file open error!\n");
        	    	exit(0);
    	    	}
    		    if (flock(fp->_fileno, LOCK_EX) != 0) 
				...
				ofstream fout("/home/w/huarzail/CMP_POPNET/pt_net_3.txt", ios::app);
				fout << sim_cycle << " " << popnetCostTime << " " << s1 << " "<< s2 << " " << src_addr << " " << d1 << " " << d2 << " "  << dst_addr << " " << data_length << " " << operation << " " << msgNo <<" ";			
					 for(i=0; i<data_length; i++)
					 {
				     fout << mailbox[i] << " ";
