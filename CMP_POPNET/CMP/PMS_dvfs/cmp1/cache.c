...
int dir_fifo_enqueue(struct DIRECTORY_EVENT *event, int type)
{
    ...
    else
    {
	    ...
	    (event->operation == Mailboxsend)		||
	    (event->operation == Mailboxreceive)//zl
            )
        {
        ...
        if (cmd == Write)
        {
            ...
            for (cnt = 0; cnt < numcontexts; cnt++)
            {
                if ((quiesceAddrStruct[cnt].address == addr))
                {
                    ...
                    sleepTimeEnd = sim_cycle;

                    sleepTime += (sleepTimeEnd - quiesceAddrStruct[cnt].sleepTimeStart);
                }
            }
        }
#ifdef  L1_STREAM_PREFETCHER
