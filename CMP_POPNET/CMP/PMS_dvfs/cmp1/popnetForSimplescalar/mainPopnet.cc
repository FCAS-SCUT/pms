		...
		for(int i = 0; i < fileLine; i++){
			...
			if(ConvertStringToNum(filecontent[i],numResult)){	
				...
				if(operation_t == 1){ //operation equal to 1 is stand for other clusters want to write this cluster.
					...
					write_memory(1,(md_addr_t)0x1ff700000LL, pt_signal_wl);
				}
					...
					fout << event_time_t <<" " <<Time << " "  << d[0] <<" " << d[1] <<" " << dst_addr_t <<" " << s[0] <<" " << s[1] <<" " << src_addr_t <<" " << data_length_t <<" " << operation_t <<" " << messageNo_t <<" ";      
					...
					fout_b << event_time_t <<" " <<Time << " " << d[0] <<" " << d[1] <<" " << dst_addr_t <<" " << s[0] <<" " << s[1] <<" " << src_addr_t <<" " << data_length_t <<" " << operation_t <<" "<< messageNo_t<< " ";
