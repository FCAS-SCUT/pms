...
extern mess_struct_type MessageInfo[2000];//ljh
extern int countNum;//ljh
extern char* writeFile;
extern char* writeFileBackup;
//extern void read_memory(int mailbox[], long data_length, md_addr_t src_addr);
//extern void write_memory(int data_length, md_addr_t dst_addr, int mailbox[]);
			...
			time_type d, long e, long vs)
{
	...
	//if(a == 7)
    // if (c[0] ==0&& c[1] == 0)
	//	cout << "Entery_inject - Time: " << mess_queue::m_pointer().current_time() << "Router ID: " << address_[0] << address_[1] << "Message no: " << a << endl;
	...
	for(long l = 0; l < e; l++) {
		...
		counter_dec(0, vs);
		for(long i = 0; i < flit_size_; i++) {
			init_data_[i] = static_cast<Atom_type>(init_data_[i] * CORR_EFF_ + 
										SRGen::wrg().flat_ull(0, MAX_64_));
...
{
//	if(c.flit_id() == 7)
   //  if (c.des_addr()[0] ==0&& c.des_addr()[1] == 0)
...
void sim_router_template::flit_traversal(long i)
{
	...
	//	if(flit_t.flit_id() == 7)
    //if (flit_t.des_addr()[0] ==0 && flit_t.des_addr()[1] == 0)
...
int sim_router_template::accept_flit(time_type a, const flit_template & b)
{
	...
	if(b.type() == TAIL_ || b.flit_size() == 1)
	{
		...
		counter_t simNetCycle = 0;
	    counter_t insertNetCycle = 0;
		int src_cmp;
		...
		long long int stTime = b.start_time();
		...
		if(y==0&&z==0)
		{
			for(j=0; j<countNum; j++)
				if(MessageInfo[j].src1 == w && MessageInfo[j].src2 == x && MessageInfo[j].messageNo_m == msgNo)
				{
				    insertNetCycle   = MessageInfo[j].sim_cycle_m;
				    src_cmp          = MessageInfo[j].src_cmp_m;
				    src_addr         = MessageInfo[j].src_addr_m;
				    dst_cmp	         = MessageInfo[j].dst_cmp_m;
				    dst_addr         = MessageInfo[j].dst_addr_m;
				    data_length      = MessageInfo[j].data_length_m;
				    operation        = MessageInfo[j].operation_m;
			
				FILE *fp_o;
				FILE *fp_b; // buckup the file
				fp_o = fopen (writeFile, "a+");
				fp_b = fopen (writeFileBackup, "a+");
					
				while(fp_o == NULL){
					cout << "package for popnet is not ready!!" << endl;
					assert(0);
				}
				while(fp_b == NULL){
					cout << "package for popnet is not ready!!" << endl;
                ...
                simNetCycle = sim_cycle - insertNetCycle;

                if (flock(fp_o->_fileno, LOCK_EX) != 0) 
                    printf("file pt_net_in.txt lock by others\n");
                if (flock(fp_b->_fileno, LOCK_EX) != 0) 
                    printf("file pt_net_in_backup.txt lock by others\n");
					  
				if(operation == 0){ 
					if(dst_cmp == 1){        	
						fprintf(fp_o, "%llu %llu 0 0 %lld 0 1 %lld %d %d %d ",sim_cycle,simNetCycle, src_addr, dst_addr, data_length, operation, msgNo);
						for(i=0; i<data_length; i++)
						{
							fprintf(fp_o,"%d ",read_data); 
						}
						fprintf(fp_o,"\n");
						fflush(stdout);  

						fprintf(fp_b, "%llu %llu 0 0 %lld 0 1 %lld %d %d %d ",sim_cycle,simNetCycle, src_addr, dst_addr, data_length, operation, msgNo);
						for(i=0; i<data_length; i++)
						{
							fprintf(fp_b,"%d ",read_data); 
						}
						fprintf(fp_b,"\n");
						fflush(stdout);           
					}
   		        else if(dst_cmp == 2){   
				fprintf(fp_o, "%llu %llu 0 0 %lld 1 0 %lld %d %d %d ",sim_cycle,simNetCycle,src_addr, dst_addr, data_length, operation, msgNo);
                for(i=0; i<data_length; i++)
                {
                    fprintf(fp_o,"%d ",read_data); 
                }
				...
				fprintf(fp_b, "%llu %llu 0 0 %lld 1 0 %lld %d %d %d ",sim_cycle, simNetCycle, src_addr, dst_addr, operation, data_length, msgNo);
                for(i=0; i<data_length; i++)
                {
                    fprintf(fp_b,"%d ",read_data); 
                }
				...
				fprintf(fp_o, "%llu %llu 0 0 %lld 1 1 %lld %d %d %d ",sim_cycle,simNetCycle,  src_addr, dst_addr, data_length, operation, msgNo);
                for(i=0; i<data_length; i++)
                {
                    fprintf(fp_o,"%d ",read_data); 
                }
				...
				fprintf(fp_b, "%llu %llu 0 0 %lld 1 1 %lld %d %d %d ",sim_cycle,simNetCycle, src_addr, dst_addr, data_length, operation, msgNo);
                for(i=0; i<data_length; i++)
                {
                    fprintf(fp_b,"%d ",read_data); 
                }
		    ...
		    fclose(fp_o);
	        fclose(fp_b);
            flock(fp_o->_fileno, LOCK_UN);             
            flock(fp_b->_fileno, LOCK_UN);             
				}
				else if(operation == 1){  //write
					read_memory(mailbox, data_length, src_addr);                 
					if(dst_cmp == 1){  
						fprintf(fp_o, "%llu %llu 0 0 %lld 0 1 %lld %d %d %d ",sim_cycle, simNetCycle, src_addr, dst_addr, data_length, operation, msgNo);
						for(i=0; i<data_length; i++)
						{
							fprintf(fp_o,"%d ",mailbox[i]); 
						}
						fprintf(fp_o,"\n");
						fflush(stdout);  
				...
				fprintf(fp_b, "%llu %llu 0 0 %lld 1 0 %lld %d %d %d ",sim_cycle, simNetCycle, src_addr, dst_addr, data_length ,operation,msgNo,simNetCycle);
                for(i=0; i<data_length; i++)
                {
                    fprintf(fp_b,"%d ",mailbox[i]); 
                }
	...
	//	ofstream fout("../../../cmp0.txt", ios::app);
	//	fout << a <<" " << w << " " << x <<" " << y <<" " << z <<" " << msgNo << endl;
	//	fout.close();
