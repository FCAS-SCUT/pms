 ...
 *       verison of it, i.e., if the PC axor address component is only
...
bpred_create (enum bpred_class aclass,	/* type of predictor to create */
	      unsigned int bimod_size,	/* bimod table size */
	      unsigned int l1size,	/* 2lev l1 table size */
	      unsigned int l2size,	/* 2lev l2 table size */
	      unsigned int meta_size,	/* meta table size */
	      unsigned int shift_width,	/* history register width */
	      unsigned int axor,	/* history xor address flag */
	      ...
	      unsigned int retstack_size)	/* num entries in ret-addr stack */
{
	...
	/* 2-level component */
	pred->dirpred.twolev = bpred_dir_create (BPred2Level, l1size, l2size, shift_width, axor);
    ...
    case BPred2Level:
	pred->dirpred.twolev = bpred_dir_create (aclass, l1size, l2size, shift_width, axor);

	break;

#ifdef LOAD_PREDICTOR
    case LDPred2bit:
	pred->dirpred.bimod = bpred_dir_create (aclass, bimod_size, 0, 0, 0);
...
bpred_dir_create (enum bpred_class aclass,	/* type of predictor to create */
...
{
    switch (pred_dir->aclass)
    {
    case BPred2Level:
	fprintf (stream,
		 "pred_dir: %s: 2-lvl: %d l1-sz, %d bits/ent, %s axor, %d l2-sz, direct-mapped\n", name, pred_dir->config.two.l1size, pred_dir->config.two.shift_width, pred_dir->config.two.axor ? "" : "no", pred_dir->config.two.l2size);
	break;
    ...
    )
{
    ...
    if (combined2 || (pred->aclass == BPredARVI))
    {
...
#ifdef LOAD_PREDICTOR
    if (!(MD_OP_FLAGS (op) & F_CTRL) && (pred->aclass != LDPred2bit))
	return 0;
#else
...
*/
    ...
    if (pred->aclass == BPredNotTaken || pred->aclass == BPredTaken)
	return;


#ifdef LOAD_PREDICTOR
    if (pred->aclass == LDPred2bit)
    {				/* update state  */
    ...
    if (!hist_imm && (MD_OP_FLAGS (op) & (F_CTRL | F_UNCOND)) != (F_CTRL | F_UNCOND) && (pred->aclass == BPred2Level || pred->aclass == BPredComb))
    {
    ...
    else if (!hist_imm && (MD_OP_FLAGS (op) & (F_CTRL | F_UNCOND)) != (F_CTRL | F_UNCOND) && (pred->aclass == BPredComb2))
    {
	int l1index, shift_reg;
...
/*lechen changed the behavior for  2bcgskew and comb2 by adding the 1 following line*/
    if (pred->aclass != BPred2bcgskew && pred->aclass != BPredComb2)
    {
	/* 
